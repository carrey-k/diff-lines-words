...
let test = require('testit');
let diff_match_patch = require('../');
let DIFF_DELETE = diff_match_patch.DIFF_DELETE;
let DIFF_INSERT = diff_match_patch.DIFF_INSERT;
let DIFF_EQUAL = diff_match_patch.DIFF_EQUAL;
...
function testDiffCharsToLines() {
  ...
  dmp.diff_charsToLines_(diffArray, ['', 'alpha\n', 'beta\n']);
  assertEquivalent(
    [
      [DIFF_EQUAL, 'alpha\nbeta\nalpha\n'],
      [DIFF_INSERT, 'beta\nalpha\nbeta\n'],
  ...
  assertEquals(n, chars.length);
  lineList.unshift('');
  let diffArray1 = [[DIFF_DELETE, chars]];
  dmp.diff_charsToLines_(diffArray1, lineList);
  assertEquivalent([[DIFF_DELETE, lines]], diffArray1);
  ...
  diffArray1 = [[DIFF_INSERT, results.chars1]];
  dmp.diff_charsToLines_(diffArray1, results.lineArray);
  assertEquals(chars, diffArray1[0][1]);
}
...
function testDiffCleanupMerge() {
  ...
  // Null case.
  let diffArray = [];
  dmp.diff_cleanupMerge(diffArray);
  assertEquivalent([], diffArray);
  ...
  dmp.diff_cleanupMerge(diffArray);
  assertEquivalent([[DIFF_EQUAL, 'abc']], diffArray);
  ...
  dmp.diff_cleanupMerge(diffArray);
  assertEquivalent([[DIFF_DELETE, 'abc']], diffArray);
  ...
  dmp.diff_cleanupMerge(diffArray);
  assertEquivalent([[DIFF_INSERT, 'abc']], diffArray);
...
function testDiffCleanupSemanticLossless() {
  ...
  // Null case.
  let diffArray = [];
  dmp.diff_cleanupSemanticLossless(diffArray);
  assertEquivalent([], diffArray);
...
function testDiffPrettyHtml() {
  ...
  assertEquals('<span>a&para;<br></span><del style="background:#ffe6e6;">&lt;B&gt;b&lt;/B&gt;</del><ins style="background:#e6ffe6;">c&amp;d</ins>', dmp.diff_prettyHtml(diffArray));
}
...
function testDiffText() {
  ...
  assertEquals('jumps over the lazy', dmp.diff_text1(diffArray));

  assertEquals('jumped over a lazy', dmp.diff_text2(diffArray));
}
...
function testDiffDelta() {
  ...
  let text1 = dmp.diff_text1(diffArray);
  assertEquals('jumps over the lazy', text1);

  let delta = dmp.diff_toDelta(diffArray);
  assertEquals('=4\t-1\t+ed\t=6\t-3\t+a\t=5\t+old dog', delta);

  // Convert delta string into a diff.
  assertEquivalent(diffArray, dmp.diff_fromDelta(text1, delta));
  ...
  text1 = dmp.diff_text1(diffArray);
  assertEquals('\u0680 \x00 \t %\u0681 \x01 \n ^', text1);

  delta = dmp.diff_toDelta(diffArray);
  ...
  diffArray = [[DIFF_INSERT, "A-Z a-z 0-9 - _ . ! ~ * ' ( ) ; / ? : @ & = + $ , # "]];
  let text2 = dmp.diff_text2(diffArray);
  ...
  // Convert delta string into a diff.
  assertEquivalent(diffArray, dmp.diff_fromDelta('', delta));
...
function testPatchObj() {
  // Patch Object.
  let p = new diff_match_patch.patch_obj();
  p.start1 = 20;
  p.start2 = 21;
  p.length1 = 18;
  p.length2 = 17;
...
function testPatchMake() {
  ...
  // Diff input.
  let diffArray = dmp.diff_main(text1, text2, false);
  patches = dmp.patch_make(diffArray);
  assertEquals(expectedPatch, dmp.patch_toText(patches));

  // Text1+Diff inputs.
  patches = dmp.patch_make(text1, diffArray);
  assertEquals(expectedPatch, dmp.patch_toText(patches));

  // Text1+Text2+Diff inputs (deprecated).
  patches = dmp.patch_make(text1, text2, diffArray);
  assertEquals(expectedPatch, dmp.patch_toText(patches));
  ...
  assertEquivalent(diffArray, dmp.patch_fromText("@@ -1,21 +1,21 @@\n-%601234567890-=%5B%5D%5C;',./\n+~!@#$%25%5E&*()_+%7B%7D%7C:%22%3C%3E?\n")[0].diffs);
...
function testPatchSplitMax() {
  ...
  let oldToText = dmp.patch_toText(patches);
  dmp.patch_splitMax(patches);
